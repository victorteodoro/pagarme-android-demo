...
public class LoginActivity extends Activity implements LoaderCallbacks<Cursor> {
  ...
  public static final String STRING = "Problema na conexão!";
  ...
  private void attemptLogin() {
    ...
    if (cancel) {
      ...
      authTask(email, password);
    }
  }

  private void authTask(String email, String password) {
    ...
    Call<UserInfo> call = apiService.createSession(credentials);
    call.enqueue(new Callback<UserInfo>() {
      @Override public void onResponse(Call<UserInfo> call, Response<UserInfo> response) {
        ...
        if(response.isSuccessful()) {
          ...
          // Go to the nest activity passing the session_id and encryption_key
          final String sessionId = response.body().getSessionId();
          final String encryptionKey = response.body().getCompanyEncryptionKey().getTest();
          final String softDescriptor = response.body().getCompanyName();
      ...
      @Override public void onFailure(Call<UserInfo> call, Throwable t) {
        Toast.makeText(LoginActivity.this, getString(R.string.error_no_connexion), Toast.LENGTH_LONG).show();
      }
    });
  }
  ...
  private boolean isPasswordValid(String password) {
    //TODO: Replace this with a better checking system
